...
export default function SetupWizard() {
  ...
  const handleFinish = async () => {
    ...
    if (appConfig.isDevelopment) {
        console.warn('Modo desenvolvimento: Configuração não será salva no banco.');
        navigate('/dashboard'); // Navega mesmo sem salvar
        return;
      }

    try {
      const success = await systemConfigService.saveConfig({
        ...config,
        is_configured: true,
      ...
      case 4:
        return (
          <div className="space-y-4">
            ...
            {selectedPlan.price && (
              <button
                onClick={() => {/* Implementar integração com Pagar.me */}}
                className="mt-8 w-full p-4 bg-green-500 text-white rounded text-lg"
              >
